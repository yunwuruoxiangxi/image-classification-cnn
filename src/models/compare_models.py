...
def compare_models():
    ...
    # 損失對比
    axes[0].plot(mlp_history['test_loss'], 'b--', label='MLP', linewidth=2, alpha=0.7)
    ...
    # 準確率對比
    axes[1].plot(mlp_history['test_acc'], 'b--', label='MLP', linewidth=2, alpha=0.7)
    ...
    print(f"\n✅ 對比圖表已保存：./experiments/results/model_comparison.png")
    plt.show()

    # 分析
    print("\n📈 分析：")
    print(f"   1. CNN 比 MLP 準確率高 {max(cnn_history['test_acc']) - max(mlp_history['test_acc']):.2f}%")
    print(f"   2. CNN 收斂更快，損失下降更明顯")
    print(f"   3. CNN 更適合圖像分類任務")

    print("\n💡 原因：")
    print(f"   - CNN 使用卷積操作，能捕捉空間特徵")
    print(f"   - MLP 將圖像展平，丟失空間信息")
    print(f"   - CNN 參數共享，減少過擬合")
